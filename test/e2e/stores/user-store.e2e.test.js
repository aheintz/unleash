...
test.serial('should set password_hash for user', async t => {
    ...
    await store.setPasswordHash(user, 'rubbish');
    const hash = await store.getPasswordHash(user);

    t.is(hash, 'rubbish');
});

test.serial('should not get password_hash for unknown email', async t => {
    const store = stores.userStore;
    const error = await t.throwsAsync(
        async () => {
            await store.getPasswordHash({ email: 'unknown@mail.com' });
        },
...
test.serial('should not get password_hash for unknown user', async t => {
    const store = stores.userStore;
    const error = await t.throwsAsync(
        async () => {
            await store.getPasswordHash({ username: 'unknown' });
        },
        { instanceOf: NotFoundError },
    );
